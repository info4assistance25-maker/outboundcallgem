...
export function Step1Upload() {
  ...
  const downloadAppointmentTemplate = () => {
    const ws = XLSX.utils.aoa_to_sheet([
      ...
      ['Giulia Bianchi', '+393334567890', '22/06/2026', '11:30', 'Igiene dentale'],
    ]);
    ws['!cols'] = [{ wch: 20 }, { wch: 18 }, { wch: 14 }, { wch: 10 }, { wch: 22 }];
    const wb = XLSX.utils.book_new();
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-soft overflow-hidden">
      ...
      <div className="flex bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
        {[
          ...
          { id: 'appuntamenti', icon: FileSpreadsheet, label: 'Conferma Appuntamenti' },
        ].map((tab) => {
        ...
        {uploadMode === 'appuntamenti' && (
          <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 px-4 py-3 rounded-xl text-sm font-medium text-blue-700 dark:text-blue-300">
              <strong>Formato richiesto:</strong> Colonne <code>Nome</code>, <code>Numero</code>, <code>Data</code> (gg/mm/aaaa), <code>Ora</code> (HH:MM), <code>Prestazione</code> (opzionale)
            </div>
            <div
            ...
            <div className="flex items-center justify-between text-xs text-slate-500">
              <span>5 colonne: Nome | Numero | Data | Ora | Prestazione (opzionale)</span>
            ...
            {contacts.length > 0 && (
              <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                <div className="grid grid-cols-[1fr_1fr_0.9fr_0.7fr_1.2fr_40px] bg-slate-50 dark:bg-slate-800/80 p-3 border-b border-slate-200 dark:border-slate-700 gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                  <span>Nome</span><span>Numero</span><span>Data</span><span>Ora</span><span>Prestazione</span><span />
                </div>
                <div className="max-h-[280px] overflow-y-auto">
                  {contacts.map(c => (
                    <div key={c.id} className="grid grid-cols-[1fr_1fr_0.9fr_0.7fr_1.2fr_40px] gap-2 px-3 py-2 border-b border-slate-100 dark:border-slate-800 last:border-0 items-center hover:bg-slate-50 dark:hover:bg-slate-800/40">
                      <input type="text" placeholder="Mario Rossi" value={c.nome} disabled={isViewer}
                        onChange={e => updateAppointmentRow(c.id, 'nome', e.target.value)}
                      ...
                      <input type="text" placeholder="+39..." value={c.numero} disabled={isViewer}
                        onChange={e => updateAppointmentRow(c.id, 'numero', e.target.value)}
                      ...
                      <input type="text" placeholder="gg/mm/aaaa" value={c.data_appuntamento || ''} disabled={isViewer}
                        onChange={e => updateAppointmentRow(c.id, 'data_appuntamento', e.target.value)}
                        className="w-full bg-white dark:bg-slate-900 px-2 py-1.5 text-sm rounded-lg border border-slate-300 dark:border-slate-600 focus:border-brand-500 focus:outline-none text-slate-900 dark:text-slate-100" />
                      <input type="text" placeholder="HH:MM" value={c.ora_appuntamento || ''} disabled={isViewer}
                        onChange={e => updateAppointmentRow(c.id, 'ora_appuntamento', e.target.value)}
                        className="w-full bg-white dark:bg-slate-900 px-2 py-1.5 text-sm rounded-lg border border-slate-300 dark:border-slate-600 focus:border-brand-500 focus:outline-none text-slate-900 dark:text-slate-100" />
                      <div className="relative">
                        <input type="text" placeholder="es. Visita cardiologica" value={c.prestazione || ''} disabled={isViewer}
                          list={`prestazione-list-${c.id}`}
                          onChange={e => updateAppointmentRow(c.id, 'prestazione', e.target.value)}
                          className="w-full bg-white dark:bg-slate-900 px-2 py-1.5 text-sm rounded-lg border border-slate-300 dark:border-slate-600 focus:border-brand-500 focus:outline-none text-slate-900 dark:text-slate-100" />
                        <datalist id={`prestazione-list-${c.id}`}>
                          <option value="Visita di controllo" />
                          <option value="Visita cardiologica" />
                          <option value="Visita ortopedica" />
                          <option value="Visita dermatologica" />
                          <option value="Igiene dentale" />
                          <option value="Prima visita" />
                          <option value="Prelievo del sangue" />
                          <option value="Ecografia" />
                          <option value="Radiografia" />
                        </datalist>
                      </div>
                      {!isViewer && (
                        <button onClick={() => deleteManualRow(c.id)}
                          className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      )}
            ...
            {!isViewer && (
              <button onClick={handleManualAppointmentAdd}
...
export function Step2Preview() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-soft overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="p-4">
        <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
          <div className={`grid ${isAppointment ? 'grid-cols-[1fr_1fr_auto_auto_1fr_auto]' : 'grid-cols-[1fr_1fr_auto]'} bg-slate-50 dark:bg-slate-800/80 p-3 border-b border-slate-200 dark:border-slate-700 gap-3 text-xs font-bold text-slate-500 uppercase tracking-wider`}>
            <span>Nome</span>
            <span>Numero</span>
            {isAppointment && <span>Data</span>}
            {isAppointment && <span>Ora</span>}
            {isAppointment && <span>Prestazione</span>}
            <span>Stato</span>
          </div>
          <div className="max-h-[220px] overflow-y-auto">
            {filteredContacts.slice(0, 50).map(c => (
              <div key={c.id} className={`grid ${isAppointment ? 'grid-cols-[1fr_1fr_auto_auto_1fr_auto]' : 'grid-cols-[1fr_1fr_auto]'} gap-3 p-3 items-center border-b border-slate-100 dark:border-slate-800/50 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/30 text-sm`}>
                ...
                {isAppointment && <span className="text-slate-400 text-xs truncate">{c.prestazione || '—'}</span>}
